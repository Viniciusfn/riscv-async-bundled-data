...
if { ${SYNC_VERSION} == 0 } {
    set_input_delay  -max ${MEM_IO_PERIOD} -clock [get_clocks {CAPTURE_PC_FD}] [get_ports {i_inst}]
    set_input_delay  -min 0.001           -clock [get_clocks {CAPTURE_PC_FD}] [get_ports {i_inst}]

    set_output_delay -max ${OUT_IO_MAX_DELAY} -clock [get_clocks {CAPTURE_PC_FD}] [get_ports {o_pc}]
    set_output_delay -min 0.001          -clock [get_clocks {CAPTURE_PC_FD}] [get_ports {o_pc}]

    set_input_delay  -max ${MEM_IO_PERIOD} -clock [get_clocks {CAPTURE_EM_MW}] [get_ports {i_readData}]
    set_input_delay  -min 0.001           -clock [get_clocks {CAPTURE_EM_MW}] [get_ports {i_readData}]
    set_output_delay -max ${OUT_IO_MAX_DELAY} -clock [get_clocks {CAPTURE_EM_MW}] [get_ports {o_mem_clk o_writeData o_writeAddr o_memWrite o_writeWidth}]
    set_output_delay -min 0.001           -clock [get_clocks {CAPTURE_EM_MW}] [get_ports {o_mem_clk o_writeData o_writeAddr o_memWrite o_writeWidth}]
